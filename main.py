...
async def download_video_with_ytdlp(
    ...
    """Download public video/web video URLs through yt-dlp.

    This is used only when a normal HTTP request does not return a direct
    video stream. It supports sites such as YouTube and many HLS/DASH/web
    video pages without changing the PDF flow.
    """
    try:
        import yt_dlp
    ...
    before = {x.resolve() for x in Path(output_dir).glob("*") if x.is_file()}
    ...
    def hook(data):
        ...
        if status == "downloading":
            ...
            downloaded = int(data.get("total_bytes") or data.get("downloaded_bytes") or 0)
            total = downloaded
        else:
            return
        try:
    ...
    def run_download():
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            return str(prepared) if prepared else ""

    prepared_path = await loop.run_in_executor(None, run_download)
    ...
    if prepared_path:
        ...
        if prepared.exists() and prepared.stat().st_size > 0:
            candidates.insert(0, prepared)

    # Deduplicate while keeping preferred prepared path first.
    unique = []
    seen = set()
    ...
    return {
        ...
        "size": size,
        "content_type": "video/mp4" if final_path.suffix.lower() == ".mp4" else "video/*",
        "final_url": url,
    }
