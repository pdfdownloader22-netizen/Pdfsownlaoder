...
async def download_file(
    ...
    """Download a resource with the exact pdf-vvv proxy flow plus robust fallbacks.

    PDF flow:
      1) SEGMENT proxy -> inspect response.
      2) If JSON contains pdf_url, extract it.
      3) SEGMENT proxy -> extracted URL.
      4) If proxy keeps returning JSON, try the extracted URL directly.
      5) Accept only a real PDF (%PDF-) for PDF requests.
    """
    ...
    async with aiohttp.ClientSession(
        ...
        final_headers = build_headers(current_url)
        probe_note = ""

        if is_pdf_request and PDF_PROXY_URL:
            # ====================================================
            # EXACT PDF-VVV STYLE RESOLUTION
            # ====================================================
            seen = set()
            resolved = None
            proxy_repeated = False
            ...
            for attempt in range(6):
                ...
                async with session.get(
                    ...
                    if resp.status >= 400:
                        raise RuntimeError(
                            f"PDF proxy HTTP {resp.status}"
                        )

                    ct = resp.headers.get(
                        "Content-Type", ""
                    ).lower()
                    first = await resp.content.read(
                        2 * 1024 * 1024
                    )

                    if not first:
                        raise RuntimeError(
                            "PDF proxy returned an empty response"
                        )

                    # Real PDF returned by proxy.
                    if first.startswith(b"%PDF-"):
                        final_request_url = proxy_url
                        final_headers = headers
                        resolved = current_url
                        break

                    # Decode JSON exactly as pdf vvv.html does.
                    payload = None
                    if (
                        "json" in ct
                        or "text/plain" in ct
                    ...
                    if extracted and valid_url(str(extracted)):
                        ...
                        # The endpoint sometimes returns the same JSON URL
                        # repeatedly. Do not keep proxying the same URL forever.
                        ...
                        probe_note = (
                            f"API resolved on attempt {attempt + 1}"
                        )
                        continue
                    ...
                    raise RuntimeError(
                        "PDF proxy returned neither a PDF nor a valid "
                        f"pdf_url. Response: {preview}"
                    )
            else:
                raise RuntimeError(
                    "PDF URL resolution exceeded 6 attempts"
                )

            # If SEGMENT keeps wrapping the endpoint in JSON, try the
            # extracted/static URL directly. This is the permanent fallback
            # for the nested-json case seen with some AppX endpoints.
            if resolved is not None and (
                proxy_repeated
                or final_request_url == original_url
            ):
                direct_candidates = []
                if current_url:
                    direct_candidates.append(current_url)
                if resolved and resolved not in direct_candidates:
                    direct_candidates.append(resolved)

                direct_ok = False
                for direct_url in direct_candidates:
                    direct_headers = build_headers(direct_url)
                    ...
                    async with session.get(
                        ...
                        if direct_resp.status >= 400:
                            continue

                        direct_first = await direct_resp.content.read(
                            2 * 1024 * 1024
                        )
                        if direct_first.startswith(b"%PDF-"):
                            final_request_url = direct_url
                            final_headers = direct_headers
                            current_url = direct_url
                            direct_ok = True
                            break

                        # A direct URL can itself be an API wrapper. Extract it
                        # and try the final URL directly on the next pass.
                        try:
                        ...
                        if isinstance(direct_payload, dict):
                            nested = (
                                direct_payload.get("pdf_url")
                                or direct_payload.get("url")
                            )
                            if nested and valid_url(str(nested)):
                                nested = str(nested).strip()
                                if nested != direct_url:
                                    nested_headers = build_headers(nested)
                                    nested_headers["Accept"] = (
                                        "application/pdf,application/octet-stream,*/*;q=0.8"
                                    )
                                    async with session.get(
                                        nested,
                                        ...
                                        if nested_resp.status < 400:
                                            nested_first = await nested_resp.content.read(
                                                2 * 1024 * 1024
                                            )
                                            if nested_first.startswith(b"%PDF-"):
                                                final_request_url = nested
                                                final_headers = nested_headers
                                                current_url = nested
                                                direct_ok = True
                                                break

                if not direct_ok and proxy_repeated:
                    raise RuntimeError(
                        "PDF source is returning nested JSON instead of the actual PDF. "
                        "The final pdf_url could not be fetched as a PDF."
                    )
        ...
        async with session.get(
            ...
            # One last nested-JSON guard. Never save JSON/HTML as .pdf.
            if is_pdf_request and not first_chunk.startswith(b"%PDF-"):
                try:
                    payload = json.loads(
                        first_chunk.decode(
                            "utf-8", errors="replace"
                ...
                if nested and valid_url(str(nested)):
                    raise RuntimeError(
                        "PDF source returned nested JSON pdf_url instead of PDF. "
                        "The source endpoint did not expose the actual PDF."
                    )
                ...
                raise RuntimeError(
                    "Actual PDF नहीं मिला. Server response: "
                    f"{preview}"
                )
            ...
            if is_pdf_request:
                ...
                if (
                    ...
                    and original_name.lower().endswith(".pdf")
                ):
...
async def process_url(
    ...
    try:
        ...
        caption = (
            ...
            f"┃ 📦 Size: <b>{result['size'] / 1024 / 1024:.2f} MB</b>\n"
            "┃ ✅ Status: <b>Successfully Downloaded</b>\n"
            "╰━━━━━━━━━━━━━━━━━━━━━━╯"
        )
...
# ============================================================
# FORCE JOIN + MAIN KEYBOARD (RESTORED)
# ============================================================

async def force_join_ok(user_id, context):
    """Return True when force-join is disabled or the user joined all channels."""
    if not get_setting("force_join_enabled", False):
        return True
    ...
    for channel in channels:
        try:
            ...
            logger.warning("Force join check failed: %s", exc)
            return False

    return True


async def require_force_join(update, context):
    """Show join buttons when the user has not joined required channels."""
    user = update.effective_user
    if not user:
        return False

    if await force_join_ok(user.id, context):
        return True
    ...
    await update.effective_message.reply_text(
        "🔒 <b>Access Required</b>\n\n"
        "Bot use करने से पहले required channel join करें।",
        parse_mode="HTML",
...
def main_keyboard():
    """Build the main user keyboard from current settings."""
    rows = []

    if get_setting("download_pdf_enabled", True):
        rows.append([
            InlineKeyboardButton("📄 PDF Download", callback_data="download_pdf")
        ])

    if get_setting("download_video_enabled", True):
        rows.append([
            InlineKeyboardButton("🎥 Video Download", callback_data="download_video")
        ])
    ...
    if get_setting("channel_upload_enabled", True):
        rows.append([
