...
async def download_file(
    ...
    async with aiohttp.ClientSession(
        ...
        # --------------------------------------------------------
        # PDF/API RESOLUTION FLOW
        # pdf vvv.html first sends the URL through SEGMENT.
        # Some AppX/course URLs return JSON containing pdf_url
        # instead of the PDF itself. Resolve that JSON URL and
        # send the extracted URL through SEGMENT again.
        # This was the missing step in the bot.
        # --------------------------------------------------------
        ...
        for _ in range(4):
            async with session.get(
                ...
                # JSON/plain API response: extract pdf_url/video_url/url
                # exactly like pdf vvv.html does.
                if (
                    is_pdf_request
                    and chunk
                    and (
                    ...
                    if extracted and valid_url(str(extracted)):
                        resolved_original_url = str(extracted)
                        current_request_url = (
                            PDF_PROXY_URL
                            + quote(
                ...
                # We have the actual binary response. Keep its first chunk
                # and stream the remainder below.
                response = resp
                first_chunk = chunk
                content_type = ct

                # IMPORTANT: the response context cannot remain open after
                # this block, so save the response headers and then fetch
                # the binary one final time for streaming.
                response_headers = dict(resp.headers)
                final_url = str(resp.url)
                break
        else:
            raise RuntimeError(
                "PDF/API resolution failed after 4 attempts"
            )

        # Re-open the final binary URL so the entire file can be streamed.
        async with session.get(
            current_request_url,
            allow_redirects=True,
        ) as response:
            ...
            first_chunk = await response.content.read(1024 * 1024)
            if not first_chunk:
                raise RuntimeError("PDF response is empty")

            if is_pdf_request and not first_chunk.startswith(b"%PDF-"):
                # One more JSON resolution attempt in case the endpoint
                # changed its response between the probe and stream request.
                try:
                    import json
                    payload = json.loads(
                ...
                if extracted and valid_url(str(extracted)):
                    raise RuntimeError(
                        "PDF endpoint returned another JSON pdf_url. "
                        "Retry the URL once; nested API response detected."
                    )
            ...
            # Store the final resolved URL for logging/name decisions.
            final_response_url = str(response.url)

            # The proxy URL normally has no useful filename.
            # Prefer the final/original PDF URL filename.

            filename = filename_from_response(response)

            # The proxy URL normally has no useful filename.
            # Prefer the final/original URL filename for PDFs.
            if is_pdf_request:
